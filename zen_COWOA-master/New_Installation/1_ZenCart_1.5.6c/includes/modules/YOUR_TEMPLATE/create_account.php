...
if (isset($_POST['action']) && ($_POST['action'] == 'process')) {
  ...
  if ($error == true) {
    ...
    $cowoa_account = $db->Execute("SELECT customers_id, customers_default_address_id FROM " . TABLE_CUSTOMERS . " 
                                     ...
                                     LIMIT 1;");
    if (!$cowoa_account->EOF) { 
      // cowoa account exists, use that
      $db_action = 'update';
      $_SESSION['customer_id'] = $cowoa_account->fields['customers_id'];
      $customers_default_address_id = $address_id = $cowoa_account->fields['customers_default_address_id'];
    ...
    $sql_data_array = array(array('fieldName'=>'customers_firstname', 'value'=>$firstname, 'type'=>'stringIgnoreNull'),
                           ...
                           array('fieldName'=>'customers_default_address_id', 'value'=>$customers_default_address_id, 'type'=>'integer'),
    ...
    if (!$cowoa_account->EOF) { 
       $sql_data_array[] = array('fieldName'=>'COWOA_account', 'value'=>0, 'type'=>'integer'); 
    }
    ...
    $db->perform(TABLE_CUSTOMERS, $sql_data_array, $db_action, $db_customers_where);
    ...
    if ($db_action == 'update') {
      $sql_data_array[] = array('fieldName'=>'address_book_id', 'value'=>$address_id, 'type'=>'integer');
      $db_address_table_where = 'address_book_id = "' . $address_id . '"';
    } else {
      $db_address_table_where = '';
    }

    $db->perform(TABLE_ADDRESS_BOOK, $sql_data_array, $db_action, $db_address_table_where);
