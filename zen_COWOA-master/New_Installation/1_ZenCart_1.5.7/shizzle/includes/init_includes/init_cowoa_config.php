...
//$messageStack->add('COWOA v2.7 install started','success');

/* Updated for COWOA 2.6 on 02/03/2015 */
// updated by bislewl 2/24/2015
global $sniffer;
...
$cowoa_menu_text = 'COWOA Settings';
...
/* find if COWOA old (prior to v2.6 COWOA) configuration title exists */
...
/* find if COWOA old (prior to v2.6 COWOA) configuration description exists */
...
$sql = "INSERT IGNORE INTO " . TABLE_CONFIGURATION . " (configuration_id, configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added, use_function, set_function) VALUES (NULL, 'Banner Display Groups Shop with Confidence', 'SHOW_BANNERS_GROUP_CHECKOUT_CONFIDENCE', 'shop-confidence', 'For \"Shop with Confidence\" box on Login page', '" . $cowoa_configuration_id . "', 22, NOW(), NULL, NULL)";
$db->Execute($sql);

$sql = "INSERT IGNORE INTO " . TABLE_CONFIGURATION . " (configuration_id, configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added, use_function, set_function) VALUES (NULL, 'Enable Customer Service Checkout Page Box', 'COWOA_CS_BOX', 'true', 'Enable <strong>Customer Service</strong> Checkout Page Box?<br />Set to True so that the <strong>Customer Service</strong> checkout page box will display. The content of this checkout box is managed using the banner manager. Create a banner for this box (only one) and the system will use the banner title for the box title (suggest using <strong>Customer Service</strong>) and the content will appear in the box.', '" . $cowoa_configuration_id . "', 24, NOW(), NULL, 'zen_cfg_select_option(array(\'true\', \'false\'),')";
$db->Execute($sql);

$sql = "INSERT IGNORE INTO " . TABLE_CONFIGURATION . " (configuration_id, configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added, use_function, set_function) VALUES (NULL, 'Banner Display Groups Customer Service', 'SHOW_BANNERS_GROUP_CUSTOMER_SERVICE', 'cust-service', 'For \"Customer Service\" box content on Login page', '" . $cowoa_configuration_id . "', 26, NOW(), NULL, NULL)";
$db->Execute($sql);
...
/* CREATE SHOP WITH CONFIDENCE & CUSTOMER SERVICE BANNERS */
